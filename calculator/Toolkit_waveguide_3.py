...
class MyWaveguide:
    ...
    def __post_init__(self):
        if (self.epsilon_r1 != None) and (self.a != None) and (self.b != None):
            ...
            ellipk_0 = ellipk(k0**2)/ellipk(1-k0**2)
            c_0 = 4*const.epsilon_0*float(ellipk_0)
            
            if self.h1 != None: # substrate 1
                k1 = np.sinh(const.pi*self.a/(4*self.h1))/np.sinh(const.pi*(self.a+2*self.b)/(4*self.h1))
                ellipk_1 = ellipk(k1**2)/ellipk(1-k1**2)
            ...
            if self.h2 != None:  # substrate 2
                k2 = np.sinh(const.pi*self.a/(4*self.h2))/np.sinh(const.pi*(self.a+2*self.b)/(4*self.h2))
                ellipk_2 = ellipk(k2**2)/ellipk(1-k2**2)
                if ellipk_2 == 0:
                    ellipk_2 = -const.pi/(2*np.log(k2/4)) # Taylor expansion of ellipk_2 for small k2
