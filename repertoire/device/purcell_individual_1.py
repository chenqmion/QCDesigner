...
def new_device(
    ...
    if ro_width_1 != 0:
        ro_height_1a = ro_length[1] - (a+2*b) - (meander_height[1] + meander_gap + r + ro_width_1 - 2*r + np.pi*r)
    else:
        ro_height_1a = ro_length[1] - (a+2*b) - (meander_height[1] + meander_gap + r)

    if purcell_width_1 != 0:
        purcell_height_1a = purcell_length[1] - (a+2*b) - (meander_height[0] + meander_gap + r + purcell_width_1 - 2 * r + np.pi * r)
    else:
    ...
    cpw_2 = lambda4.new_device(length=ro_length[0]-ro_length[1], height=[(meander_height[0] + meander_gap + r)-meander_gap,r], width=(ro_width_2, ro_width[2]), mode='incompact', a=a, b=b, r=r, d_rad=d_rad, layer=layer)
    ...
    bolometer_1 = bolometer.new_device(
        ...
        pad_gap=[[b, finger_gap * (3 * finger_number[1] - 1) * b / a], [finger_gap * b / a, finger_gap * (3 * finger_number[1] - 1) * b / a]],
        spacer=pad_spacer,
        c_g=pad_c_g,
        c_p=pad_c_p,
        sns_zone=sns_zone,
    ...
    bolometer_1_ports = cpw_1.combine_device(bolometer_1, ref=cross_2b_ports['90'].real + 1j*finger_2_ports['2'].imag, degree=0, axis='x', port='2')
