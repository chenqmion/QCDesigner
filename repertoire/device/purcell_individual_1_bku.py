...
def new_device(ro_length=[2500, 800],
    ...
    purcell_length_2 = purcell_length[0] - purcell_length[1] - (
                (purcell_width_1 - 2 * r) - (x_c - r) + (w + a + 2 * b) + np.pi * r / 2)
    cpw_2 = lambda4.new_device(length=purcell_length_2, height=[ro_height / 2, r], width=(4 * r, 2 * r),
                               mode='incompact',
                               a=a, b=b, r=r, d_rad=d_rad, layer=layer)

    cpw_2_ports = cpw_1.combine_device(cpw_2, ref=cpw_1.ports['3'] + (ro_width / 2 + (w + a + 2 * b) - 1j * r),
                                       degree=180,
                                       axis='none', port='1')
