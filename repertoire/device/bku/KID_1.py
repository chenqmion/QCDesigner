...
def new_lambda4(length_wg=6000, width_wg=600, N=0, length_c=0, gap_c=6, length_stub=0, length_load=0, a=10, b=6, r=50):
    lambda4 = device()

    if length_c == 0:
        length_c = width_wg - 2 * r

    cpw_1 = new_cpw(path=[(0, 0), (length_c, 0)])
    cpw_1.terminate_port(1, width=(a + 2 * b), gap=gap_c, degree=180)
    cpw_1_ports = lambda4.combine_device(cpw_1, port=1)

    if N == 0:
        N = int(np.floor(((length_wg - length_stub - (a / 2 + b)) - length_c) / (np.pi * r + (width_wg - 2 * r))))
    ...
    cpw_3 = new_cpw(path=[(0, 0), (r, 0), (r, -length_3)])
    if N % 2 == 0:
    ...
    # lambda4.add_port(2, cpw_3_ports['2'])
...
def new_junction(width_gap=4, height_gap=14,
    ...
    poly_1 = []
    poly_1.append((-width_nanowire / 2, length_nanowire / 2))
    poly_1.append((-width_nanowire / 2, length_nanowire / 2 + 2 * over_etch))
    poly_1.append((width_nanowire / 2, length_nanowire / 2 + 2 * over_etch))
    poly_1.append((width_nanowire / 2, length_nanowire / 2))
    ...
    poly_1 = []
    poly_1.append((-width_nanowire / 2, length_nanowire / 2 + 2 * over_etch))
    poly_1.append((-width_nanowire / 2, length_nanowire / 2 + 3 * over_etch))
    poly_1.append((width_nanowire / 2, length_nanowire / 2 + 3 * over_etch))
    poly_1.append((width_nanowire / 2, length_nanowire / 2 + 2 * over_etch))
    ...
    poly_1.append((width_nanowire / 2, length_nanowire / 2))
    poly_1.append((width_nanowire / 2, length_nanowire / 2 + width_patch))
    poly_1.append((width_nanowire / 2 + 2 * over_etch, length_nanowire / 2 + width_patch))
    poly_1.append((width_nanowire / 2 + 2 * over_etch, length_nanowire / 2))

    poly_2 = ope_rotate(poly_1, origin=(0, 0), degree=180)
    junction.add_geometry('EBL_1', [poly_1, poly_2])

    poly_1 = []
    poly_1.append((-width_gap / 2 - 3 * over_etch, length_nanowire / 2))
    poly_1.append((-width_gap / 2 - 3 * over_etch, length_nanowire / 2 + width_patch))
    poly_1.append((-width_gap / 2 - 2 * over_etch, length_nanowire / 2 + width_patch))
    poly_1.append((-width_gap / 2 - 2 * over_etch, length_nanowire / 2))
    poly_2 = ope_rotate(poly_1, origin=(0, 0), degree=180)
    ...
    poly_3.append((width_nanowire / 2 + 3 * over_etch, length_nanowire / 2 + width_patch))
    poly_3.append((width_nanowire / 2 + 3 * over_etch, length_nanowire / 2))
...
def new_KID(length_wg=6000, width_wg=600, N=0, length_c=0, gap_c=6, length_stub=100, length_load=100,
